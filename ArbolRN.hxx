...
template<class T>
int arbolRN<T>::altura(NodoBinario<T>* nodo) {
    int valt = -1;
    if (nodo->esHoja()) <{
        valt = 0;
    } else {
...
template<class T>
int arbolRN<T>::tamano(NodoBinario<T>* nodo) {
    if (nodo == NULL) {
        return 0;
    }
...
template<class T>
NodoBinario<T>* arbolRN<T>::balancear(NodoBinario<T>* nodo) {
    if (!nodo) return nodo;
...
template<class T>
NodoBinario<T>* arbolRN<T>::insertar(NodoBinario<T>* nodo, T val) {
...
template<class T>
NodoBinario<T>* arbolRN<T>::eliminar(NodoBinario<T>* nodo, T val) {
    if (!nodo) return nodo;
    if (val < nodo->obtenerDato()) {
